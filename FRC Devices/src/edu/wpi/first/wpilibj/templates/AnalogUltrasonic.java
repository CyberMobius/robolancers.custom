...
/**
 * Class representing a generic ultrasonic rangefinder that gives an analog input
 * given its conversion from voltage to units of distance.
 * 
 * @author Eric Lam 
 * 
 */
public class AnalogUltrasonic extends AnalogChannel {
    ...
    public AnalogUltrasonic(int channel, DistanceUnits units, double scaleFactor) {
        this(getDefaultAnalogModule(), channel, units, scaleFactor);
    }
     ...
     * For example, the LV-MaxSonar-EZ1 from the 2012 FRC KOP's scaling factor is 9.8 mV/in. with a 5V supply. 
     */
    public AnalogUltrasonic(int slot, int channel, DistanceUnits units, double scaleFactor) {
        super(slot, channel);
    ...
    public double getDistance() {
        ...
        if (units == DistanceUnits.CENTIMETERS) {
            distance = (double) (getVoltage() * 1000) / scaleFactorCm;
        } else if (units == DistanceUnits.INCHES) {
            distance = (double) (getVoltage() * 1000) / scaleFactorIn;
        }
    ...
    public double getAverageDistance() {
        ...
        if (units == DistanceUnits.CENTIMETERS) {
            distance = (double) (getAverageVoltage() * 1000) / scaleFactorCm;
        } else if (units == DistanceUnits.INCHES) {
            distance = (double) (getAverageVoltage() * 1000) / scaleFactorIn;
        }
